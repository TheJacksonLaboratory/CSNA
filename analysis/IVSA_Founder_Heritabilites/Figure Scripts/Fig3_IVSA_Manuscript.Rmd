...
CC_extinction_slope = CC_extinction_df %>%
  group_by(DE_Subject, Lever) %>%
    do(tidy(lm(lever_presses ~ Session, .)))%>%
...
```
Preparing the data to asess strain differences in Slope of extinction with CC and founder strains, combined. Seperating the two active and inactive lever into two distinct column. Looking for strain differences in slope of extinction on active lever and inactive lever.
```{r}
CC_extinction_slope_wide <- CC_extinction_slope %>%
  filter(DE_Strain != "J:DO")%>% 
  filter(DE_Strain %in% c(Combined_key_abv)) %>%
spread(Lever, Slope)
Ex_model_A <- anova(lm(A ~ DE_Strain, data = CC_extinction_slope_wide))
Ex_model_A
Ex_model_I <- anova(lm(I ~ DE_Strain, data = CC_extinction_slope_wide))
Ex_model_I
```

Preparing the data to asess strain differences in Slope of extinction with only CC strains. Seperating the two active and inactive lever into two distinct column. Looking for strain differences among CC strains.
...
Preparing the data to asess strain differences in Slope of extinction with only founder strains. Seperating the two active and inactive lever into two distinct column.Looking for strain differences among founder strains.

```{r}
CC_extinction_slope_wide <- CC_extinction_slope %>% 
  filter(DE_Strain != "J:DO")%>% 
  filter(DE_Strain %in% c(founders_key$strain)) %>% 
spread(Lever, Slope)
Ex_model_A <- anova(lm(A ~ DE_Strain, data = CC_extinction_slope_wide))
Ex_model_A
Ex_model_I <- anova(lm(I ~ DE_Strain, data = CC_extinction_slope_wide))
...
print(model3)
```
Working with founder and CC strains to asess strain difference in extinction. Generating a data table for active lever presses to do a repeated measures ANOVA over the seven extinction sessions.

```{r extinction_plot, warning=FALSE}
CC_EX_ALP_df = CC_founders_IVSA %>%
  filter(DE_Strain != "J:DO")%>% 
  filter(DE_Strain %in% c(Combined_key_abv)) %>%
  select(DE_Subject,
         DE_Strain,
...
Preparing a dataframe for CC and founder active lever press during extinction with complete observations for all seven extinction sessions.
```{r}
library(plyr)
CC_EX_ALP_df <- ddply(CC_EX_ALP_df,.(DE_Subject),transform,count=length(Session))

#View(CC_EX_ALP_df)
CC_EX_ALP_df_completeobs <- subset(CC_EX_ALP_df, count == 7)
#View(CC_EX_pAP_df_completeobs)
```
Summarizing the resulting dataframe for extinction  that contains complete observations for all seven extinction sessions.
```{r}
detach(package:plyr)
library(dplyr)
CC_EX_ALP_df_summary = CC_EX_ALP_df %>% group_by(DE_Strain, DE_Sex, Lever, Session,count) %>%
  summarize(mean = mean(lever_presses, na.rm = TRUE),
            n = length(which(!is.na(lever_presses))),
            sem = sd(lever_presses, na.rm = TRUE) / sqrt(length(which(!is.na(lever_presses))))) %>%
  mutate(ul = mean + sem, ll = mean - sem)
CC_EX_ALP_df_summary
```

Performing ANOVA to test the influence of strain on Active lever Presses with a repeated measures design.

```{r}
CC_EX_ALP_df_completeobs$Session <- as.factor(CC_EX_ALP_df_completeobs$Session)   


model3 =ezANOVA(
  data = CC_EX_ALP_df_completeobs,
  dv = lever_presses,
  wid = DE_Subject,
  within = .(Session),
  between = .(DE_Strain),
...
```{r}
library(plyr)
CC_EX_ILP_df <- ddply(CC_EX_ILP_df,.(DE_Subject),transform,count=length(Session))

#View(CC_EX_ALP_df)
CC_EX_ILP_df_completeobs <- subset(CC_EX_ILP_df, count == 7)
#View(CC_EX_pAP_df_completeobs)
```
Summarizing the resulting dataframe for extinction  that contains complete observations for all seven extinction sessions.
```{r}
detach(package:plyr)
library(dplyr)
CC_EX_ILP_df_summary = CC_EX_ILP_df %>% group_by(DE_Strain, DE_Sex, Lever, Session,count) %>%
  summarize(mean = mean(lever_presses, na.rm = TRUE),
            n = length(which(!is.na(lever_presses))),
            sem = sd(lever_presses, na.rm = TRUE) / sqrt(length(which(!is.na(lever_presses))))) %>%
  mutate(ul = mean + sem, ll = mean - sem)
CC_EX_ILP_df_summary
```

Performing ANOVA to test the influence of strain on Percent Active lever Presses with a repeated measures design.

```{r}
CC_EX_ILP_df_completeobs$Session <- as.factor(CC_EX_ILP_df_completeobs$Session)   


model3 =ezANOVA(
  data = CC_EX_ILP_df_completeobs,
  dv = lever_presses,
  wid = DE_Subject,
  within = .(Session),
  between = .(DE_Strain),
