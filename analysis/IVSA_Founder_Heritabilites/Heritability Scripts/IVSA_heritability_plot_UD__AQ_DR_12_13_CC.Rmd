...
Getting founders key and preparing a CC strain key. Getting the CC only (no CC or DO) for heritabiliy calculations. 
...
IVSA_data <- IVSA_data [grep("^CC", IVSA_data$Strain), ]

#IVSA_data = IVSA_data %>%
#  filter((!(Strain %in% founders_key$strain)) & Strain != "J:DO")

CC_key = IVSA_data %>% distinct(Strain)%>% rename(strain = Strain)
CC_key$strain = factor(CC_key$strain)
CC_key$collaborative_cross_color_broman = rep("#CCCCCC", times = nrow(CC_key))
```
...
ggplot(data = IVSA_resid, aes(x = Strain, y = DR_Inf_Total_1p0mgkg_Resid, color = Strain, shape = Sex)) +
  ...
  scale_color_manual(values = CC_key$collaborative_cross_color_broman) +
...
The residuals indicate heteroscedasticity.Doing a Box-Cox to see if a response variable transformation is merited.
...
IVSA_DR_1p0mgkg_summary$Strain_Sex = factor(IVSA_DR_1p0mgkg_summary$Strain_Sex,
                                                levels = paste(rep(CC_key$strain, each = 2), rep(c(" F", " M"), times = 44), sep = ""),
                                                ordered = TRUE)
...
This does not look better. Trying Box-Cox to see if the log-likelihood is centered at 1 with the transformed data.

```{r}
boxcox(lm(log10(AQ_SessionsToAcquisition) ~ Strain, data = IVSA_resid))
```

Trying 1/ sqrt(x) transformation. 

```{r}
boxcox(lm(AQ_SessionsToAcquisition ~ Strain, data = IVSA_resid))
```

The log-likelihood centered at - 0.5 implies that a 1/sqrt(x) may help with the non-normal residuals.

```{r}
IVSA_resid$AQ_SessionsToAcquisition_Resid_1_sqrt = resid(lm((1/sqrt(AQ_SessionsToAcquisition)) ~ Strain, data = IVSA_resid))

ggplot(data = IVSA_resid, aes(x = Strain, y = AQ_SessionsToAcquisition_Resid_1_sqrt, color = Strain, shape = Sex)) +
  geom_hline(yintercept = 0, col = "#999999") +
  geom_beeswarm(size = 2) + 
  scale_color_manual(values = CC_key$collaborative_cross_color_broman) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="none") +
...
```{r}
boxcox(lm((1/sqrt(AQ_SessionsToAcquisition)) ~ Strain, data = IVSA_resid))
```


Proceeding with model selection using the log10 transformation.
...
IVSA_data$Strain_Sex = factor(paste(as.character(IVSA_data$Strain),
                              ...
                              levels = paste(rep(CC_key$strain, each = 2), rep(c("F","M"), times = 44), sep = " "),
                              ordered = TRUE)
IVSA_data$Strain = factor(IVSA_data$Strain, levels = CC_key$strain, ordered = TRUE)

CC_FR1_individual_points_plot = ggplot(data = IVSA_data, aes(x = Strain_Sex, y = DR_Inf_Total_1p0mgkg, color = Strain, shape = Sex)) +
  geom_beeswarm(size = 2) +
  scale_color_manual(values = c(CC_key$collaborative_cross_color_broman)) +
  scale_shape_manual(values = c(19,17)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="none") +
  xlab(NULL) +
  ylab("FR1 Total Infusions at 1.0 mg/kg") +
  ggtitle("IVSA: 1.0 mg/kg Infusions")
CC_FR1_individual_points_plot
```

Plotting individual points for IVSA sessions to acquisition

```{r}
CC_StA_individual_points_plot = ggplot(data = IVSA_data, aes(x = Strain_Sex, y = AQ_SessionsToAcquisition, color = Strain, shape = Sex)) +
  geom_beeswarm(size = 2) +
  scale_color_manual(values = c(CC_key$collaborative_cross_color_broman)) +
  scale_shape_manual(values = c(19,17)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="none") +
  ...
  ylab("Sessions to Acquisition") +
  ggtitle("IVSA: Sessions to Acquisition (FR1 1.0 mg/kg)")
CC_StA_individual_points_plot
```

Saving plot

```{r}
saveRDS(IVSA_AQ_SessionsToAcquisition_plot, "C:/Users/dattau/Documents/IVSA_Proj_org/IVSA/IVSA/csna_project_4_behavior/output/IVSA_AQ_SessionsToAcquisition_strain_sex_plot.RDS")
```
...
IVSA_DR_Inf_Total_AUC_summary$Strain_Sex = factor(IVSA_DR_Inf_Total_AUC_summary$Strain_Sex,
                                                levels = paste(rep(CC_key$strain, each = 2), rep(c(" F", " M"), times = 38), sep = ""),
                                                ordered = TRUE)
