    ...
    data  = cc.data %>%
      ...
      extremesCC[[i]] <- list(both   = data %>% mutate(Sex =  "Both"),
        ...
        map(function(x){
              ...
              #plot
              p <- ggplot(ys, aes(y = mean, x = reorder(StrainSig, -diff_mean), 
                                  color = sig, label = sig)) + 
                geom_point() + 
...
### 7, Correlation in CC and/or Founders
```{r Correlation in CC and/or Founders, warning = FALSE, fig.width=16, fig.height=16}
#correlation in CC-------------------------
...
```{r, CC Extreme Strain Survey, warning = FALSE, fig.width=12, fig.height=12}
...
for(d in names(extremesCC.res)){
  extremesCC.res[[d]] %>% 
    modify_depth(2, ~print(.x[[2]])) #print in rmarkdown
  #print into pdf
  pdf(file = paste0("output/pdf/", d, ".extremesCC.pdf"), width = 14, height = 10) 
    extremesCC.res[[d]] %>% modify_depth(2, ~print(.x[[2]]))
  dev.off()
}
```

### 9, DO phenos and correlation
```{r DO phenos and correlation, warning = FALSE}
...
sens <- sens %>% rename(Mouse.ID = mouseID) %>%
...
colnames(sens)[4:9] <- paste0(colnames(sens)[4:9], ".raw")
colnames(sens)[4:15] <- paste0("SENS.", colnames(sens)[4:15])
...
### 9, Phenotypes for QTL
```{r Phenotypes for QTL, warning = FALSE}
