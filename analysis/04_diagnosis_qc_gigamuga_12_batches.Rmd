...
#iplot
setScreenSize(height=100, width=150)
...
pdf(file = "output/Proportion_matching_genotypes_before_removal_samples.pdf", width = 20, height = 20) 
...
pdf(file = "output/Proportion_matching_genotypes_after_removal_samples_percent_missing_5.pdf",width = 20, height = 20) 
...
qc_info <- qc_info %>%
  mutate(remove.id.duplicated = case_when(
    id %in% unique(c(filtered.summary.cg$ind1, filtered.summary.cg$ind2)) ~ TRUE,
    !(id %in% unique(c(filtered.summary.cg$ind1, filtered.summary.cg$ind2))) ~ FALSE
  ))
